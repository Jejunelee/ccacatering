...
import { updateBlogPostBySlug, getBlogPostBySlugAdmin, isUserAdmin } from "@/lib/blog";
...
export default function EditBlogPostPage() {
  ...
  useEffect(() => {
    const loadPostData = async () => {
      // Don't load if already loaded or component unmounted
      ...
      try {
        ...
        // IMPORTANT: Check admin status here instead of relying on context
        ...
        // Load post data
        const { data: post, error: postError } = await getBlogPostBySlugAdmin(postSlug);
        
        if (!isMounted.current) return;
        
        if (postError || !post) {
          setError(postError?.message || "Blog post not found");
          setIsLoading(false);
          return;
        }
    ...
    // Add a small delay to ensure params are ready
    const timer = setTimeout(loadPostData, 100);
  ...
  // Show loading only if we're actually loading AND have params to work with
  const showLoading = isLoading || !params?.slug;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const isUserActuallyAdmin = await isUserAdmin();
      if (!isUserActuallyAdmin) {
        throw new Error("You must be an admin to update posts");
      }

      // Use current slug from params (should be available by now)
      const currentSlug = params?.slug as string;
      if (!currentSlug) {
        throw new Error("Unable to determine which post to update");
      }

      const { error: updateError } = await updateBlogPostBySlug(currentSlug, formData);
        ...
        <div className="max-w-4xl mx-auto px-4">
          <div className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-gray-600">
              {!params?.slug ? "Loading blog post..." : "Loading blog post data..."}
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="max-w-4xl mx-auto px-4">
          <div className="bg-red-50 border border-red-200 rounded-2xl p-8 text-center">
            ...
            <div className="flex gap-4 justify-center">
              ...
              <Link
                href="/blog"
                className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-orange-500 to-amber-500 text-white rounded-xl font-medium hover:opacity-90 transition-opacity"
              >
      ...
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="mt-8 p-6 bg-red-50 rounded-2xl border border-red-200">
          ...
          <p className="text-sm text-red-800 mb-4">
            Deleting this post cannot be undone. All associated data will be permanently removed.
          </p>
          <button
            type="button"
            onClick={() => {
              if (confirm("Are you absolutely sure you want to delete this post? This action cannot be undone.")) {
                ...
                // You would call deleteBlogPost here
              }
            }}
